...
describe("Z-score against WHO reference values", () => {
  it("hits the median at birth", () => {
    // WHO weight-for-age, girls, day 0: median 3.2322 kg
    expect(zScore("weight", "female", 0, 3.2322)).toBeCloseTo(0, 3);
    // Boys, day 0: median 3.3464 kg
    expect(zScore("weight", "male", 0, 3.3464)).toBeCloseTo(0, 3);
    // Length-for-age, girls, day 0: median 49.1477 cm
    expect(zScore("length", "female", 0, 49.1477)).toBeCloseTo(0, 3);
  });

  it("hits the published standard deviations at birth", () => {
    // WHO table girls day 0: −2 SD = 2.395 kg, +2 SD = 4.230 kg
  ...
  it("hits the median at one year", () => {
    // WHO weight-for-age, girls, 365 days: median around 8.9 kg
    const z = zScore("weight", "female", 365, 8.9);
    expect(z).not.toBeNull();
    expect(Math.abs(z!)).toBeLessThan(0.1);

    // Length-for-age, girls, 365 days: median around 74 cm
    const zl = zScore("length", "female", 365, 74);
    expect(Math.abs(zl!)).toBeLessThan(0.2);
  });

  it("distinguishes boys and girls", () => {
...
describe("valueAtZ is the inverse of zScore", () => {
  it("round-trips for every percentile line", () => {
  ...
  it("returns monotonically rising percentile curves", () => {
    // P3 < P15 < P50 < P85 < P97 — on every day.
...
describe("zToPercentile", () => {
  it("maps known z-scores onto their percentiles", () => {
...
describe("Edge cases", () => {
  it("clamps ages outside the table to its edges", () => {
  ...
  it("interpolates between the weekly anchor points", () => {
